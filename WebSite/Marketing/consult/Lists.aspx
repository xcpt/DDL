<%@ Page Language="C#" AutoEventWireup="true" Inherits="Marketing_consult_Lists" %>
<%if (action == "view")
  { %>
<div class="Rtop">
    <div class="RlmM">咨询未到店:</div>
</div>
<div class="Tgnk">
	<div class="Rtss" style="padding-bottom:5px;">
	  ...
	  <div style="float:left"><input type="button" name="button" id="SearchButton" value="搜 索" onclick="$('#consultGrid').flexReload(ReadInputValue('SearchInputValue'));" icon="icon-zoom" /></div>
	</div>
</div>
<div class="RtbK">
    <table id="consultGrid" style="display: none;">
    </table>
</div>
<script type="text/javascript">
    SearchEnter("SearchButton","SearchInputValue");
    var gridList= $("#consultGrid").flexigrid
    (
    {
        url: 'Marketing/consult/Lists.aspx',
        newp:<%=page %>,
        dataType: 'json',
        colModel: [
        ...
        autoload:true,
        <%if(page=="1"){%>PageOneValue:<%Response.Write(Web.UI.customer.User.View_consult(StoresID, starttime, endtime)); %>,<%}%>
...
<%}
  ...
  {
      Response.Write(Web.UI.customer.User.View_consult(StoresID, starttime, endtime));
  }%>