<%@ Page Language="C#" AutoEventWireup="true" Inherits="Sys_Stores_Modi" %>
<%if(!action.Equals("save")){ %>
<div id="StoresModi">
<input type="hidden" name="id" value="<%=sModel.storesid%>" />
<table class="table StyleView" border="0" cellspacing="0" scellpadding="0" width="100%">
    <tr>
        ...
        <td align="left">
            <input type="text" class="validate[required,length[1,250]]" name="title" value="<%=sModel.title %>" id="storestitle" size="50" maxlength="250" />
        </td>
    </tr>
    <tr>
        <th align="right">
        ...
        <td align="left">
            <input type="radio" value="1" name="Isoutlets" id="Isoutlets_1"<%=Base.Fun.fun.isChecked("1",sModel.Isoutlets) %> /><label for="Isoutlets_1">直营</label>（<input type="checkbox" value="1" name="IsCross" <%=Base.Fun.fun.isChecked("1",sModel.IsCross) %> id="IsCross" /><label for="IsCross">跨店消费</label>）
            <input type="radio" value="0" name="Isoutlets" id="Isoutlets_0"<%=Base.Fun.fun.isChecked("0",sModel.Isoutlets) %> /><label for="Isoutlets_0">加盟</label>
        </td>
    </tr>
    <tr>
        <th width="80" align="right">
        ...
        <td align="left">
            <input type="text" class="validate[required]" value="<%=Base.Fun.fun.Get_Date(DateTime.Parse(sModel.opentime),"YY04-MM-DD") %>" name="opentime" id="storesopentime" onClick="WdatePicker({ skin: 'ext' })" size="10" readonly="readonly" maxlength="50" />
        </td>
    </tr>
    <tr>
        <th width="80" align="right">
        ...
        <td align="left">
            <select class="validate[required,length[1,20]]" name="Province" id="province"></select>&nbsp;<select class="validate[required,length[1,20]]" name="City" id="city"></select>
        </td>
    </tr>
    <tr>
        <th width="80" align="right">
        ...
        <td align="left"><input type="text" class="validate[required,length[1,250]]" value="<%=sModel.address %>" name="address" id="storesaddress" size="50" maxlength="250" /></td>
    </tr>
    <tr>
        <th width="80" align="right">
            上班时间：
        ...
        <td align="left">
			早：<input type="text" name="Worktime" id="storesWorktime" value="<%=sModel.Worktime %>" size="5" />&nbsp;
            晚：<input type="text" name="Closingtime"  id="storesClosingtime" value="<%=sModel.Closingtime %>" size="5"/>
        </td>
    </tr>
    <tr>
        <th width="80" align="right">
        ...
        <td align="left"><input type="text" class="validate[required,length[1,50]]" name="tel" id="tel" value="<%=sModel.tel %>" size="50" maxlength="50" /></td>
    </tr>
</table>
</div>
<script type="text/javascript">
    ...
    document.getElementById("province").onchange = select;
    function init(pname, cname) {
        var pi = 0;
        with (document.getElementById("province")) {
            length = where.length;
            for (k = 0; k < where.length; k++) {
                if (pname == where[k].loca) {
                    pi = k;
                }
        ...
        document.getElementById("province").selectedIndex = pi;
        var ci = 0;
        with (document.getElementById("city")) {
            loca3 = (where[pi].locacity).split("|");
            length = loca3.length;
            for (l = 0; l < length; l++) {
                if (cname == loca3[l]) {
                    ci = l;
        ...
        document.getElementById("city").selectedIndex = ci;
    }
    init('<%=sModel.Province %>', '<%=sModel.City %>');
</script>
<%} %>