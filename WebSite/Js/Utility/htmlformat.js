    ...
    function Parser() {
        ...
        this.get_content = function() { //function to capture regular content between tags
            ...
            while (this.input.charAt(this.pos) !== '<') {
                ...
                schar = this.input.charAt(this.pos);
                this.pos++;
                this.line_char_count++;


                if (this.Utils.in_array(schar, this.Utils.whitespace)) {
                    if (content.length) {
                        space = true;
                    }
                    this.line_char_count--;
        ...
        this.get_tag = function() { //function to get a full tag and parse its type
            ...
            do {
                ...
                if (this.Utils.in_array(schar, this.Utils.whitespace)) { //don't want to insert unnecessary space
                    space = true;
                    this.line_char_count--;
                    continue;
                }
                ...
                if (schar === "'" || schar === '"') {
                    if (!content[1] || content[1] !== '!') { //if we're in a comment strings don't get treated specially
                        schar += this.get_unformatted(schar);
                        space = true;
                    }
                }

                if (schar === '=') { //no space before =
                    space = false;
                }

                if (content.length && content[content.length - 1] !== '=' && schar !== '>'
            && space) { //no space after = or before >
                ...
                content.push(schar); //inserts character at-a-time (or string)
            } while (schar !== '>');
