...
//拖放程序
var CutPicDrag = Class.create();
CutPicDrag.prototype = {
  //拖放对象,触发对象
  initialize: function(obj, drag, options) {
    var oThis = this;
	
	this._obj = $$$$(obj);//拖放对象
	this.CutPicDrag = $$$$(drag); //触发对象
	...
	addEventHandler(this.CutPicDrag, "mousedown", function(e){ oThis.Start(window.event || e); });
  },
  //设置默认属性
  ...
  Start: function(oEvent) {
	...
	//使鼠标移到窗口外也能释放
	if(isIE){
		addEventHandler(this.CutPicDrag, "losecapture", this._fS);
		this.CutPicDrag.setCapture();
	}else{
  ...
  Stop: function() {
	...
	if(isIE){
		removeEventHandler(this.CutPicDrag, "losecapture", this._fS);
		this.CutPicDrag.releaseCapture();
	}else{
...
ImgCropper.prototype = {
    ...
    initialize: function (container, drag, url, width, height, options) {
        ...
        //拖放对象
        this.CutPicDrag = $$$$(drag);
        this.CutPicDrag.style.cursor = "move";
        this.CutPicDrag.style.zIndex = 200;
        if (isIE) {
            //设置overflow解决ie6的渲染问题(缩放时填充对象高度的问题)
            this.CutPicDrag.style.overflow = "hidden";
            //ie下用一个透明的层填充拖放对象 不填充的话onmousedown会失效(未知原因)
            (function (style) {
                style.width = style.height = "100%"; style.backgroundColor = "#fff"; style.filter = "alpha(opacity:0)";style.opacity = 0;
            })(this.CutPicDrag.appendChild(document.createElement("div")).style)
        }
        ...
        this.ViewSize = $$$$(this.options.ViewSize) || null;
        ...
        this.Scale = this.options.Scale;
        this.viewComparison = parseInt(this.options.viewComparison);
        //设置拖放
        this._drag = new CutPicDrag(this.CutPicDrag, this.CutPicDrag, { Limit: true, onMove: function () { oThis.SetPos(); } });
        //设置缩放
    ...
    Init: function () {
        var oThis = this;
        var CutWin = LoadWarting('', SycmsLanguage.Cut2Slice.l1, '');
        var oImg1 = new Image();
        oImg1.onerror = function () {
            this.onerror = null;
            CutWin.close();
            if (oThis.options.ErrorFun) {
                oThis.options.ErrorFun();
            }
        }
        oImg1.onload = function () {
            this.onload = null;
            if (oImg1.width / oThis.Width * 100 > oImg1.height / oThis.Height * 100) {
                oThis.viewComparison = Math.floor(oImg1.width / oThis.Width * 100,2);
                oImg1.height = oImg1.height * (oThis.Width / oImg1.width);
                oImg1.width = oThis.Width;
            } else {
                oThis.viewComparison = Math.floor(oImg1.height / oThis.Height * 100,2);
                oImg1.width = oImg1.width * (oThis.Height / oImg1.height);
                oImg1.height = oThis.Height;
            }
            oThis.Height = oImg1.height;
            oThis.Width = oImg1.width;
            

            //设置容器
            oThis.Container.style.width = oThis.Width + "px";
            oThis.Container.style.height = oThis.Height + "px";
            oThis.View.style.width = oThis.viewWidth + "px";
            oThis.View.style.height = oThis.viewHeight + "px";
            if (oThis.options.Scale) {
                if (oThis.dragWidth > oThis.Width) {
                    oThis.dragHeight = oThis.dragHeight * (oThis.Width / oThis.dragWidth);
                    oThis.dragWidth = oThis.Width;
                }
                if (oThis.dragHeight > oThis.Height) {
                    oThis.dragWidth = oThis.dragWidth * (oThis.Height / oThis.dragHeight);
                    oThis.dragHeight = oThis.Height;
                }
            } else {
                if (oThis.dragWidth > oThis.Width) {
                    oThis.dragWidth = oThis.Width - 2;
                }
                if (oThis.dragHeight > oThis.Height) {
                    oThis.dragHeight = oThis.Height - 2;
                }
            }
            if (oThis.dragTop + oThis.dragHeight > oThis.Height) {
                oThis.dragTop = oThis.Height - oThis.dragHeight;
            }
            if (oThis.dragLeft + oThis.dragWidth > oThis.Width) {
                oThis.dragLeft = oThis.Width - oThis.dragWidth;
            }
            oThis.dragTop = (oThis.Height - oThis.dragHeight) / 2;
            oThis.dragLeft = (oThis.Width - oThis.dragWidth) / 2;
            //设置拖放对象
            oThis.CutPicDrag.style.top = oThis.dragTop + "px";
            oThis.CutPicDrag.style.left = oThis.dragLeft + "px";
            oThis.CutPicDrag.style.width = oThis.dragWidth + "px";
            oThis.CutPicDrag.style.height = oThis.dragHeight + "px";
            //设置切割对象
            oThis._pic.src = oThis._cropper.src = oImg1.src;
            ...
            try{
                ...
                oThis._pic.style.MozOpacity = oThis.Opacity / 100;
            } catch (e) { }
            //设置预览对象
            if (oThis.View) { oThis._view.src = oImg1.src; }
        ...
        oImg1.src = oThis.Url + "?i=" + Math.random();
    },
    //设置获取缩放对象
    GetResize: function () {
        var op = this.options;
        ...
        if (op.RightDown || op.LeftDown || op.RightUp || op.LeftUp || op.Right || op.Left || op.Up || op.Down) {
            var oThis = this, _resize = new Resize(this.CutPicDrag, { Limit: true, onResize: function () { oThis.SetPos(); } });
            //设置缩放触发对象
    ...
    //设置切割
    SetPos: function () {
        var o = this.CutPicDrag;
        //按拖放对象的参数进行切割
        if (o.offsetWidth != 0) {
            this.dragTop = o.offsetTop;
            this.dragLeft = o.offsetLeft;
            this.dragWidth = o.offsetWidth;
            this.dragHeight = o.offsetHeight;
        }
        this._cropper.style.clip = "rect(" + this.dragTop + "px " + (this.dragLeft + this.dragWidth) + "px " + (this.dragTop + this.dragHeight) + "px " + this.dragLeft + "px)";
        //切割预览
    ...
    PreView: function () {
        //按比例设置宽度和高度
        var o = this.CutPicDrag;
        var ooffsetHeight = o.offsetHeight;
        var ooffsetWidth = o.offsetWidth;
        var ooffsetTop = o.offsetTop;
        var ooffsetLeft = o.offsetLeft;
        if (ooffsetHeight == 0) {
            ooffsetHeight = this.dragHeight;
            ooffsetWidth = this.dragWidth;
            ooffsetTop = this.dragTop;
            ooffsetLeft = this.dragLeft;
        }
        h = this.viewWidth, w = h * ooffsetWidth / ooffsetHeight;
        if (w > this.viewHeight) { w = this.viewHeight; h = w * ooffsetHeight / ooffsetWidth; }
        //获取对应比例尺寸
        var scale = h / ooffsetHeight, ph = this.Height * scale, pw = this.Width * scale, pt = ooffsetTop * scale, pl = ooffsetLeft * scale, styleView = this._view.style;
        //设置样式
        ...
        if (this.ViewSize) {
            this.ViewSize.innerHTML = SycmsLanguage.Cut2Slice.l2 + parseInt(this.dragWidth * this.viewComparison / 100) + "px；&nbsp;" + SycmsLanguage.Cut2Slice.l3 + parseInt(this.dragHeight * this.viewComparison / 100) + "px；";
        }
    }
}
